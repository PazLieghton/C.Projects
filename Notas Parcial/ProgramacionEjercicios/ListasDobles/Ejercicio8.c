...
void generarNuevoArchivoPersonalizado(void){
    ...
    while(op!=0){
    	printf("\nIngrese un codigo de procesador (maximo 8 caracteres)\n");
   		scanf("%d",&codigo);
...
int obtener_codigo_procesador(int procesador){
	int a;
	a=procesador/1000;
	printf("a vale: %d\n",a);
	return a;
...
nodo *agregar_proc_principio(nodo *lista){
	...
	scanf("%d",&codigo);
	printf("Ingrese velocidad del procesador (2 o 3 digitos maximo)\n");
	scanf("%d",&velocidad);
...
int contar_elementos(nodo *lista){
	int cant=0;
	while(lista!=NULL){
		cant++;
		lista=lista->sig;
...
nodo *crea_lista_desde_archivo(nodo *lista){
    ...
    printf("%d registros leídos desde archivo\n", cant);
   ...
   do {
      ...
      switch (op) {
         case 1:
            ...
            break;
         case 2:
         	printf("Ingrese el codigo del procesador a eliminar: \n");
         	scanf("%d",&codigo);
